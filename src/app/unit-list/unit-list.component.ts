...
export class UnitListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.service$ = this.unitService.getList().subscribe((response) => {
    ...
    this.filteredUnits$ = this.store
      ...
      .subscribe((filteredUnits) => {
        this.filteredUnits = filteredUnits.map((x) =>
          UnitService.convertToUnitDto(x),
        );
        this.changeDetector.markForCheck();
      });
    ...
    this.costFilters$ = this.store
      .select(selectCostFilters)
      .subscribe((filters) => {
  ...
  onFilterChanged() {
    this.store.dispatch(
      UnitsActions.costFilter({ costFilters: this.costFilters }),
    );
    this.store.dispatch(UnitsActions.selectPageIndex({ pageIndex: 0 }));
  }
