...
describe('RangeInputComponent', () => {
  ...
  it('should enable sliders by checkbox', async () => {
    ...
    const minRange = fixture.debugElement.nativeElement.querySelector(
      '#' + costFilter.type + '-min',
    );
    expect(minRange.getAttribute('disabled')).toBe('');
    expect(minRange.value).toBe(costFilter.min.toString());

    const maxRange = fixture.debugElement.nativeElement.querySelector(
      '#' + costFilter.type + '-max',
    );
  ...
  it('should set validate value changes', async () => {
    ...
    const minInput = fixture.debugElement.query(
      By.css('#' + costFilter.type + '-min'),
    );
    const maxInput = fixture.debugElement.query(
      By.css('#' + costFilter.type + '-max'),
    );
