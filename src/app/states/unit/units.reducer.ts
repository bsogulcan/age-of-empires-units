...
export const unitsReducer = createReducer(
  ...
  on(UnitsActions.getList, (state, { response }) => {
    const filteredUnits = UnitService.filterUnits(
      response.map((item) => Object.assign({}, item)),
      state.ageFilter.displayName,
      state.costFilters,
    );
  ...
  on(UnitsActions.ageFilter, (state, { age }) => {
    const filteredUnits = UnitService.filterUnits(
      state.units.map((item) => Object.assign({}, item)),
      age.displayName,
      state.costFilters,
    );
  ...
  on(UnitsActions.selectPageIndex, (state, { pageIndex }) => ({
    ...state,
    pageIndex,
  })),
);
